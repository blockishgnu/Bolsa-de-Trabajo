...
if(isset($_SESSION['usuario'])){
    ...
    //echo "Has iniciado sesion";
        ...
        while ($rowl=mysqli_fetch_array($res)) {
          $idreclutador=$rowl['idreclutador'];
          echo "
...
<body>
  ...
  <?php
  $result= mysqli_query($con,"SELECT * FROM sab_reclutadores WHERE username ='".$usuario."'");
   while($row=mysqli_fetch_array($result)){

   ?>

  <img src="data:image/png;base64,<?php echo base64_encode($row['logo']); ?>" alt="titulo foto"  width='150'  />
  <form enctype='multipart/form-data' action='imagen.php?idreclutador=<?=$row['idreclutador'];?>' method='POST'>
